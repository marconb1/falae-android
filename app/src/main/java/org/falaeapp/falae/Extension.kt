...
fun AlertDialog.loadDefaultShowListener() {
    setOnShowListener {
        val buttonPositive = getButton(AlertDialog.BUTTON_POSITIVE)
        val buttonNegative = getButton(AlertDialog.BUTTON_NEGATIVE)
        buttonPositive.setTextColor(ContextCompat.getColor(buttonPositive.context, R.color.colorAccent))
        buttonNegative.setTextColor(ContextCompat.getColor(buttonNegative.context, R.color.colorAccent))
    }
}